...
def main(subtask):
    ...
    #    attentions = hf_model(**tokenizer(prompt, return_tensors="pt").to("cuda"), output_attentions=True, return_dict=True).attentions
    #lm = LanguageModel(hf_model, tokenizer=tokenizer, attn_implementation="eager", device_map="auto")
    #del hf_model
    ...
    # The answer
    #answer = torch.argmax(true_logits[-1])
    ...
    for L in range(num_layers):
        ...
        #print("GATE_W SHAPE:", gate_W.shape)
        a_all_id = layer.mlp.act_fn(F.linear(mlp_in[L], gate_W))  # (T, M)  (16,8192) --> can't use this because it already represents an input (mlp_in) having been taken through the gate. but we need just the gate
        a_all = layer.mlp.act_fn(mlp_gate[L])  # (T, M)  (16,8192)
        #print("A ALL SHAPE:", a_all.shape)
        mlp_a.append(a_all)
...
def apply_mlp_token(layer_idx, tok_idx, mlp_a, layers) -> torch.Tensor:
    ...
    print("DOWN W SHAPE:", down_W.shape)
    print("UP W SHAPE:", up_W.shape)

    # Shape of keys: mlp_transformations[(layer_idx, token_idx)]
    #print("MLP A FOR THE TOKEN/LAYER TUPLE:", mlp_a[(layer_idx, tok_idx)], mlp_a[(layer_idx, tok_idx)].shape)
    #mlp_this_layer_token = mlp_a[(layer_idx, tok_idx)][:, tok_idx, :]
    mlp_this_layer_token = mlp_a[(layer_idx, tok_idx)]
    print("SHAPE OF MLPA THIS LAYER TOKEN:", mlp_a[(layer_idx, tok_idx)].shape)
    ...
    # mlp_transformation = mlp_transformation[0, :, :]

    # Test it
    # test_output = torch.einsum('... a, ... b a -> ... b', mlp_inputs[layer_idx].to(device="cuda"), mlp_transformation.to(device="cuda"))
    # to_norm = (test_output-mlp_outputs[layer_idx])[:, tok_idx, :]
...
def apply_edge_vec(layer_idx: int, 
    ...
    #print("S ATTN AL KEYS:", s_attn_all.keys())
    #print("TARG TOKEN:", target_token)
...
"""def get_logit_contrib(token, path):
    # Get the logit contribution for a single token and a single path
    x_src = X_dev[token, :].contiguous()
    transformation = apply_path_to_vec(path)
    transformed = transformation.to(device="cuda")@x_src.to(device="cuda")
    normalized = apply_final_norm_linearized(transformed)
    #answer = torch.argmax(true_logits[-1])   # defined earlier as global var
    logits = (lm_head_w@normalized).to(device="cpu")
    #percent_explained = (logits[answer]/true_logits[-1, answer])*100
    #print('Explained logit {}%'.format(percent_explained))
    return logits"""